...
実効値を確認する: 利用側コードで URL を組み立てた直後に末尾数文字をログに出すか、シェルで `echo ${DISCORD_WEBHOOK_URL: -8}` (Bash/Zsh の場合) を実行した結果と、`.env` ファイルに書かれている値の末尾とを比較する。上記「webhook URL の渡し方の注意」の衝突パターンであることが多い。
